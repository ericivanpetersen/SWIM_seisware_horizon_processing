...
class seisware_horizons(object):
	...
	def estimate_epsilon_targ_horiz(self, sub_horiz, surf_horiz, targ_horiz, filepath='./'):
		...
		out_header = ['Orbit', 'Trace', 'Latitude', 'Longitude', 'Epsilon', 'Depth (eps='+str(eps_med)+')', 'TWT surf (ns)', 'TWT sub (ns)', 'TWT targ (ns)']
		out_data = np.column_stack((self.orbit[ind], self.trace[ind], self.lat[ind], self.lon[ind], epsilon, depth, surf[ind], sub[ind], targ[ind]))
